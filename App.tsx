...
export default function App() {
  ...
  const executionSearch = async () => {
    ...
    try {
      const systemInstruction = `You are an advanced AI Search Concierge for an e-commerce website. Match the user's explicit or implicit problem to our product catalog.
      
      Store catalog: ${JSON.stringify(localDatabase)}
      User text query: "${query}"
      
      If an image is provided in the parts, analyze it closely. Recommend items that perfectly complement or solve the situation shown in the image or described in text.
      ...
      {
          "reason": "Write your detailed matching consultation advice here in plain text.",
          "matched_ids": [1, 4]
      }`;
      ...
      <div style={styles.wrapper}>
          ...
          <div style={styles.searchBarContainer}>
            <div style={styles.inputWrapper}>
              ...
              <input
                ...
                placeholder={isListening ? "🎙️ Listening to your voice..." : "Type a problem, click Mic, or upload Image..."}
                style={styles.mainInput}
              />
            </div>

            {/* ხილვადი ტექსტური ღილაკები ემოჯიებით */}
            <div style={styles.actionButtons}>
              ...
              <button 
                ...
                style={{
                  ...styles.textButton, 
                  backgroundColor: isListening ? '#textButton' : '#1e293b',
                  borderColor: isListening ? '#f43f5e' : '#334155',
                  color: isListening ? '#f43f5e' : '#cbd5e1'
                }} 
                title="Voice Search"
              >
                {isListening ? "🛑 Stop" : "🎙️ Mic"}
              </button>
              
              <button onClick={executionSearch} style={styles.submitButton} title="Search">
                Search ➔
              </button>
            </div>
          </div>
        ...
        <div style={styles.metaBar}>
          <div>
            ...
            <span style={{ color: '#64748b' }}>Scope: Global Catalog</span>
          </div>
        ...
        {/* Loader Component */}
        {isLoading && (
          <div style={styles.loaderZone}>
            <p style={{ color: '#fbbf24', fontWeight: 'bold', fontSize: '0.875rem', margin: 0 }}>⚡ Gemini is thinking...</p>
            <p style={{ color: '#64748b', fontSize: '0.75rem', marginTop: '4px', margin: 0 }}>Analyzing visual items and text context simultaneously.</p>
          </div>
        )}
